...
exports.validateBotDirectory = validateBotDirectory;
...
function validateManifest(raw) {
    ...
    else if (!types_js_1.SUPPORTED_CATEGORIES.includes(raw.category)) {
        errors.push(`Unsupported category '${raw.category}'. Allowed: ${types_js_1.SUPPORTED_CATEGORIES.join(', ')}.`);
    }
    // 6. Runtime
    if (!raw.runtime || typeof raw.runtime !== 'string') {
        errors.push("Missing or invalid 'runtime'. Must be a string.");
    }
    else if (!types_js_1.SUPPORTED_RUNTIMES.includes(raw.runtime)) {
        errors.push(`Unsupported runtime '${raw.runtime}'. Allowed: ${types_js_1.SUPPORTED_RUNTIMES.join(', ')}.`);
    }
    // 7. Entrypoint
    ...
    if (!Array.isArray(raw.credentials)) {
        ...
        raw.credentials.forEach((cred, index) => {
            ...
            if (cred.inputType && !types_js_1.VALID_INPUT_TYPES.includes(cred.inputType)) {
                errors.push(`credentials[${index}].inputType '${cred.inputType}' is invalid. Allowed: ${types_js_1.VALID_INPUT_TYPES.join(', ')}.`);
            }
        });
        if (!hasBotToken) {
    ...
    if (!Array.isArray(raw.permissions)) {
        ...
        raw.permissions.forEach((perm) => {
            if (typeof perm !== 'string' || !types_js_1.ENFORCEABLE_PERMISSIONS.includes(perm)) {
                errors.push(`Unauthorized or non-enforceable permission '${perm}'. The runtime strictly permits only: ${types_js_1.ENFORCEABLE_PERMISSIONS.join(', ')}.`);
            }
        });
    }
    return {
        ...
        errors,
        warnings,
        manifest: errors.length === 0 ? raw : undefined
    };
}
function validateBotDirectory(dirPath) {
    const errors = [];
    const warnings = [];
